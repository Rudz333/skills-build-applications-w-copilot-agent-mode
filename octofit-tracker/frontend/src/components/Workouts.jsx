...
const workoutsEndpoint = typeof window !== 'undefined'
  ? '/api/workouts/'
  : codespaceName
...
function Workouts() {
  ...
  useEffect(() => { fetchResource('workouts', workoutsEndpoint).then((data) => { setWorkouts(data); setState({ loading: false, error: '' }) }).catch((error) => setState({ loading: false, error: error.message })) }, [])
