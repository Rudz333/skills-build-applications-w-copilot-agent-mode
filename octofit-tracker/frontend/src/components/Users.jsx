...
const usersEndpoint = typeof window !== 'undefined'
  ? '/api/users/'
  : codespaceName
...
function Users() {
  ...
  useEffect(() => { fetchResource('users', usersEndpoint).then((data) => { setUsers(data); setState({ loading: false, error: '' }) }).catch((error) => setState({ loading: false, error: error.message })) }, [])
